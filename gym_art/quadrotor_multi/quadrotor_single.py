...
def compute_reward_weighted(dynamics, goal, action, dt, crashed, time_remain, rew_coeff, action_prev,
    ...
    if use_pos_diff:
        ...
        if pos_metric == 'normal':
            cost_pos = 5 - 0.3 * dist
            cost_pos = np.clip(cost_pos, a_min=0.0, a_max=5.0) * 2.0
    ...
    # e ^ (-0.5 * | | actions | |^ 2)
    cost_effort = np.exp(-0.5 * (cost_effort_raw ** 2)) * 0.01
    ...
    # max(rot, 0.0) * 0.01
    cost_orient = max(cost_orient_raw, 0.0) * 0.075
    ...
    # e ^ (- | | omega | |^ 2)
    cost_spin = np.exp(-1.0 * (cost_spin_raw ** 2)) * 0.15
...
class QuadrotorSingle:
    ...
    def __init__(self, dynamics_params="DefaultQuad", dynamics_change=None, dynamics_randomize_every=None,
                 ...
                 obst_inf_height=False, use_pos_diff=False, pos_metric='normal', spawn_height_mode=0, nearest_nbrs=0):
        ...
        self.nearest_nbrs = nearest_nbrs  # make drones and obstacles uniform obs
